...
*ã€•Server:*
> *â†’ Runtime: ${getRuntime(uptime)}*
> *â†’ Ram: ${percentRam.toFixed(2)}%*
> *â†’ CPU: ${cpuUsage.toFixed(2)}%*
> *â†’ OS:  ${os.platform()}*
...
> *â†’ .rbxstalk*`
                    ...
                    case 's':
                    case 'sticker':
                        await sock.sendMessage(from, { text: '> *ã€•Sedang membuat...*' }, { quoted: m })
                        try {
                            ...
                            if (mime === 'imageMessage' || mime === 'videoMessage' || mime === 'stickerMessage') {
                                ...
                                await sock.sendMessage(from, { text: '> *ã€•Kirim atau replay gambar.*' }, { quoted: m })
                            }
                        } catch (e) {
                            await sock.sendMessage(from, { text: '> *ã€•Terjadi masalah pada server.*' }, { quoted: m })
                        }
                        break
                    
                    case 'brat':
                        if (!text) return sock.sendMessage(from, { text: '> *ã€•Sertakan teks, contoh: .brat hello world*' }, { quoted: m })
                        await sock.sendMessage(from, { text: '> *ã€•Sedang membuat...*' }, { quoted: m })
                        try {
                    ...
                    case 'ai':
                        if (!text) return sock.sendMessage(from, { text: '> *ã€•Sertakan pertanyaan atau perintah, contoh: .ai apa itu NodeJS*' }, { quoted: m })
                        await sock.sendMessage(from, { text: '> *ã€•Sedang berfikir...*' }, { quoted: m })
                        try {
                    ...
                    case 'worm':
                        if (!text) return sock.sendMessage(from, { text: '> *ã€•Sertakan pertanyaan atau perintah, contoh: .worm apa itu NodeJS*' }, { quoted: m })
                        await sock.sendMessage(from, { text: '> *ã€•Sedang berfikir...*' }, { quoted: m })
                        try {
                    ...
                    case 'img':
                        if (!text) return sock.sendMessage(from, { text: '> *ã€•Sertakan perintah, contoh: .img buatkan saya gambar kucing.*' }, { quoted: m })
                        await sock.sendMessage(from, { text: '> *ã€•Sedang membuat gambar...*' }, { quoted: m })
                        try {
                    ...
                    case 'tourl':
                        try {
                            ...
                            if (/imageMessage|videoMessage|documentMessage/.test(mime)) {
                                await sock.sendMessage(from, { text: '> *ã€•Sedang mengupload...*' }, { quoted: m })
                                ...
                                if (result && result.length > 0) {
                                    await sock.sendMessage(from, { text: `> *ã€•Upload berhasil, link anda: ${result[0].url}*` }, { quoted: m })
                                } else {
                                    throw new Error('No url')
                                }
                            } else {
                                await sock.sendMessage(from, { text: '> *ã€•Sertakan gambar, video, atau dokumen.*' }, { quoted: m })
                            }
                        } catch (e) {
                            await sock.sendMessage(from, { text: '> *ã€•Terjadi masalah pada server.*' }, { quoted: m })
                        }
                    ...
                    case 'ssweb':
                        if (!text) return sock.sendMessage(from, { text: '> *ã€•Sertakan link dengan awalan http:// atau https://, contoh: .ssweb https://google.com*' }, { quoted: m })
                        await sock.sendMessage(from, { text: '> *ã€•Sedang mengambil gambar...*' }, { quoted: m })
                        try {
                    ...
                    case 'ttstalk':
                    case 'rbxstalk':
                        await sock.sendMessage(from, { text: '> *ã€•Fitur dalam perbaikan.*' }, { quoted: m })
                        break
